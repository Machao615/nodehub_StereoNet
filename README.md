...
**⚠️ Important Notice:**
- The C++ source code for this project comes from: https://github.com/D-Robotics/hobot_stereonet
- All content is based on the official source repository
- This repository is for documentation purposes only
- Please refer to the official source repository and RDK documentation for updates: https://developer.d-robotics.cc/rdk_doc/Robot_development/boxs/function/hobot_stereonet
- **For regular use, you can directly install the package via `apt install` without compilation**
- **Source code compilation is only required for custom development**

## ROS2 Package Compilation Tutorial

### Installation Methods

#### Method 1: Direct Installation (Recommended)
For most users, you can directly install the pre-compiled package:
...
sudo apt install tros-humble-hobot-stereonet

# If using MIPI camera
sudo apt install tros-humble-mipi-cam

# If using ZED camera
sudo apt install tros-humble-hobot-zed-cam
```

#### Method 2: Source Code Compilation (For Custom Development Only)

**⚠️ Note: Source compilation is only needed when you need to modify the algorithm or add custom features.**

1. **Prerequisites:**
   - RDK platform with Ubuntu 22.04 system
...
5. **Verification:**
   ...
   ros2 pkg list | grep hobot_stereonet
   ```

### When to Use Source Compilation

You should compile from source only when:
- You need to modify the algorithm parameters
- You want to add custom post-processing features
- You need to integrate with custom camera drivers
- You are developing new stereo vision algorithms

For regular usage, the pre-installed package via `apt install` is sufficient and recommended.
